...
class Add(Node):
    ...
    def forward(self):
        ...
        node_values = map(lambda node: node.value, self.inbound_nodes)
        out_value = sum(node_values)
...
class Linear(Node):
    ...
    def forward(self):
        ...
        prods = map(lambda pair: pair[0]*pair[1], zip(inputs, weights))
        self.value = sum(prods) + bias
