...
class SentimentNetwork:
            ...
            for word in review.split(" "):
                if total_counts[word] > min_count:
                    if word in pos_neg_ratios.keys():
                        ...
                        # other words happening less than 50 times
                        review_vocab.add(word)
        self.review_vocab = list(review_vocab)
    ...
    def train(self, training_reviews_raw, training_labels):
        ...
        for idx, review in enumerate(training_reviews_raw):
            if idx % 2500 == 0:
                print("Indexing {0}", idx)
            indices = set()
            for word in review.split(" "):
        ...
        for i in range(len(training_reviews)):
            ...
            # Input Layer
            #self.update_input_layer(training_reviews_raw[i])

            # Hidden layer
            layer_1_orig = self.layer_0.dot(self.weights_0_1)
            layer_1 = np.zeros((1, self.hidden_nodes))
            for index in review:
                # print(index)
                layer_1 += self.weights_0_1[index]
            #print("\n")
            #print("AAAA", layer_1_orig)
            #print("BBBB", layer_1)
